...
@section('content')
    ...
    <form action="{{ route('admin.posts.store') }}" method="post">
        ...
        <div class="mb-3">
            ...
            <input type="number" step="0.01" name="author" id="price" class="form-control" placeholder="Author"
                aria-describedby="authorHelper" value="{{ old('author') }}">
            <small id="authorHelper" class="text-muted">Type a author for your post</small>
            @error('author')
                <div class="alert alert-danger">{{ $message }}</div>
            @enderror
        </div>
